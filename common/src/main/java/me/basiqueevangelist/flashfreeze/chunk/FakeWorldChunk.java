...
public class FakeWorldChunk extends WorldChunk implements FakeChunk {
    ...
    public FakeWorldChunk(World world, ChunkPos pos, NbtCompound updatedTag) {
        super(world, pos, new BiomeArray(world.getRegistryManager().get(Registry.BIOME_KEY), pos, new FixedBiomeSource(BuiltinBiomes.THE_VOID)));
        this.updatedTag = updatedTag;

        for (int i = 0; i < 16; i++) {
            ChunkSection section = new ChunkSection(i * 16);
            for (int x = 0; x < 16; x++)
                for (int y = 0; y < 16; y++)
                    for (int z = 0; z < 16; z++)
    ...
    public BlockState getBlockState(BlockPos pos) {
        if (pos.getY() >= 0 && pos.getY() >> 4 < getSectionArray().length)
